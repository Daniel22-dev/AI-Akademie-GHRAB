...
const errors = [];
const passed = [];
const run = (s, a) => { console.error('DBG START', s, a.join(' ')); const r=spawnSync(process.execPath, [path.join(here, s), ...a], { encoding:'utf8', timeout:15000, env:{...process.env,GHRAB_GARP_SELFTEST:'1'} }); console.error('DBG END', s, 'status='+r.status, 'signal='+r.signal, 'error='+(r.error?.code||'')); return r; };
